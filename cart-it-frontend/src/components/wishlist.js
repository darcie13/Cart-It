...
const Wishlist = () => {
  ...
  const handleDeleteComment = async (itemId, commentId) => {
    try {
      const data = await deleteItemComment(itemId, commentId);
      const updatedItem = {
        ...data,
        notes: data.comments || data.notes,
        comments: data.comments
  ...
  // Handler function to delete an item from the wishlist
    ...
    const loadPageData = async () => {
      ...
      try {
        const [details, itemsList, sidebarData] = await Promise.all([
          ...
          getWishlistItems(id), // Fetch the list of items in the current wishlist for the user
        ...
        <section className="item-grid">
          {isLoading ? (
            ...
            sortedItems.map(item => (
              <div
                key={item.item_id}
                className={`item-card ${isEditing && selectedIds.includes(item.item_id) ? 'selected-ring' : ''} ${item.is_purchased ? 'purchased' : ''}`}
