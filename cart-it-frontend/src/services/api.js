...
// Helper for API errors that might occur
export const handleApiError = async (response, fallback) => {
...
// Fetches active cart items (purchased items excluded)
...
export const getWishlistItems = async (wishlistId) => {
  const response = await fetch(`${API_BASE}/items/wishlist/${wishlistId}`, {
    headers: { ...getAuthHeader() }
  });
...
// Deletes an existing comment from an item
export const deleteItemComment = async (itemId, commentId) => {
  const response = await fetch(`${API_BASE}/items/${itemId}/notes`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json', ...getAuthHeader() },
...
// Cart items disappear from cart view.
// Wishlist items remain visible but become greyed out.
...
// Fetches archived wishlists for archive page
export const getArchivedWishlists = async (userId) => {
  const response = await fetch(
    `${API_BASE}/wishlists/archived/all?user_id=${userId}`,
    {
      headers: { ...getAuthHeader() }
    }
  );
  ...
  if (!response.ok) {
    throw new Error('Failed to fetch archived wishlists');
  }
...
// Archives a completed wishlist
export const archiveWishlist = async (wishlistId) => {
  const response = await fetch(`${API_BASE}/wishlists/${wishlistId}/archive`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      ...getAuthHeader()
  ...
  if (!response.ok) {
    throw new Error(data.error || 'Failed to archive wishlist');
  }
...
export const deleteWishlist = async (wishlistId, userId) => {
  const response = await fetch(
    `${API_BASE}/wishlists/${wishlistId}?user_id=${userId}`,
    {
      method: 'DELETE',
      headers: { ...getAuthHeader() }
    }
...
// Permanently deletes multiple archived wishlists
export const bulkDeleteWishlists = async (ids, userId) => {
  await Promise.all(
    ids.map(id => deleteWishlist(id, userId))
  );
